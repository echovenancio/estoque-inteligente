...
class DevDBManager(GenericDBManager):
    ...
    def __create_dev_tables(self):
        ...
        if self.setted is not True:
            cursor.execute(
                "CREATE TABLE IF NOT EXISTS estoque (uuid TEXT, nm_produto TEXT, type_quantidade TEXT, val_quantidade REAL, min_quantity REAL, labels TEXT, anotation TEXT, cluster_id INTEGER, created_at TEXT, updated_at TEXT)"
            )
            cursor.execute(
                "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, email TEXT, hash_password TEXT)"
            )
            cursor.execute(
                "INSERT OR IGNORE INTO users (id, email, hash_password) VALUES (1, 'loja@email.com', '123456')"
            )
            cursor.execute(
                "INSERT OR IGNORE INTO users (id, email, hash_password) VALUES (2, 'fabrica@email.com', '123456')"
            )
            self.setted = True
        conn.commit()
        cursor.close()
    ...
    def login(self, email, password):
        ...
        cursor.execute(
            "SELECT * FROM users WHERE email = ? AND hash_password = ?",
            (email, password),
        )
        row = cursor.fetchone()
        cursor.close()
    ...
    def add_estoque(self, produto, auth_token):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO estoque (uuid, nm_produto, type_quantidade, val_quantidade, min_quantity, labels, anotation, cluster_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))",
                (
                    uuid,
                    produto.nm_produto,
                    produto.type_quantidade.lower(),
                    produto.val_quantidade,
                    produto.min_quantity,
                    json.dumps(produto.labels),
                    produto.anotation,
                    -1,
                ),
            )
    ...
    def update_estoque(self, id, produto, auth_token):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE estoque SET nm_produto = ?, type_quantidade = ?, val_quantidade = ?, min_quantity = ?, labels = ?, anotation = ?, updated_at = datetime('now') WHERE uuid = ?",
                (
                    produto.nm_produto,
                    produto.type_quantidade.lower(),
                    produto.val_quantidade,
    ...
    def get_low_stock_produtos(self, auth_token) -> list[ResProduto]:
        ...
        try:
            cursor.execute(
                "SELECT * FROM estoque WHERE val_quantidade < min_quantity AND min_quantity > 0 ORDER BY cluster_id ASC"
            )
            rows = cursor.fetchall()
            cursor.close()
        except Exception:
